...
class SphinxClient
{
	...
	function AddQuery ( $query, $index="*", $comment="" )
	{
		...
		$m_count=0;
		if($this->_weights){
		    $m_count= count($this->_weights);
		}
		$req .= pack ( "N", $m_count); // weights
		if($this->_weights){
		...
		$filterCount=0;
		if($this->_filters){
		    $filterCount=count($this->_filters);
		}
		$req .= pack ( "N",  $filterCount);
		if($this->_filters){
		...
		if($this->_indexweights){
		    $indexCount=count($this->_indexweights);
		}
		$req .= pack ( "N", $indexCount );
		if($this->_indexweights){
		...
		if($this->_fieldweights){
		    $fieldCount=count($this->_fieldweights);
		}
		$req .= pack ( "N",  $fieldCount);
		if($this->_fieldweights){
		...
		$overrideCount=0;
		if($this->_overrides){
		    $overrideCount=count($this->_overrides) ;
		}
		$req .= pack ( "N", $overrideCount);
		if($this->_overrides){
